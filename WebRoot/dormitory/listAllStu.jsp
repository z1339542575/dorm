...
<body class="bg-b">
	...
	<script type="text/javaScript">
		$("#table1").bootstrapTable({
			method : "post",
			url : "/dorm/student/getAll.do", 		// 获取表格数据的url
			//toolbar:"#toolbar",
		...
		function operateFormatter(value, row, index) {//赋予的参数
     	return [
        	'<a class="btn active btn-info btn-sm" onclick="modify(\'' + row.dorm_stu_sno + '\')">更新</a>&nbsp;&nbsp;',
			'<a class="btn active btn-info btn-sm" onclick="del(\'' + row.dorm_stu_sno + '\')">删除</a>&nbsp;&nbsp;',
			'<a class="btn active btn-info btn-sm" onclick="ban(\'' + row.dorm_stu_sno + '\')">禁用</a>', ].join('');
		}
		function modify(dorm_stu_sno) {
			window.location.href="/dorm/student/modify.do?dorm_stu_sno="+dorm_stu_sno;
		} 
		...
		function ban(dorm_stu_sno) {
			$.ajax({
				url : "/dorm/student/updateLoseState.do",
				type : "POST",
				data : {
					"dorm_stu_sno" : dorm_stu_sno
				},
		...
		function getEffeAll() {
			window.location.href = "/dorm/student/getEffeAll.do";
		}
		function refresh(){
